...
6. **Dependency major bumps.** ~~Sanity 4→5~~ ✅ done 2026-05-04 (3.70 → 5.21 stable; auto-install pulled a `@portabletext/editor` that no longer exported `coreBehaviors`, forcing the bump). Still available: TypeScript 5→6, Preact Signals 1→2. Revisit each when there's a feature reason rather than for its own sake.
...
10. ~~**Citation popover IDs duplicate when same ref cited multiple times in one doc.**~~ ✅ Resolved — `Citation.astro` accepts a per-occurrence `markKey` prop (the markDef `_key`, unique per Portable Text mark) and builds `popoverId = cite-{refId}-{markKey}`. `PortableTextSpan.astro` threads `annot._key` through. HTML now validates with multi-cited refs.
